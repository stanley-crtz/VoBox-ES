...
export const Step3 = ({
  ...
  const DefaultURLPhoto =
    "https://static.wikia.nocookie.net/blackclover/images/8/80/Black_Asta_Descontrolado.png/revision/latest?cb=20201124054144&path-prefix=es";
      ...
      Firebase.uploadPhotoProfile(photo.files[0], user.Name + new Date())
        .then((resp) => {
          resp.ref.getDownloadURL().then((url) => {
            sendingInformation({ ...user, Photo: url });
            Swal.close();
            handleNext();
          });
        })
