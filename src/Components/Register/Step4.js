...
export const Step4 = ({ sendingInformation }) => {
    ...
    const handleSubmit = (e) => {
        ...
        if (userTest(user) && photo.files[0]) {
            ...
            Firebase.uploadPhotoProfile(photo.files[0], user.Name)
                .then(resp => {
                    resp.ref.getDownloadURL()
                        .then(url => sendingInformation({...user, Photo: url}))
                });

        } else {
            Swal.fire('Advertencia', 'Debe de rellenar cada uno de los campos', 'info')
        }
            ...
            <form onSubmit={handleSubmit}>
                <div className="About">
                    <div className="AboutPhoto">
                        ...
                        <input type="file" accept="image/*" onChange={handleChangePhoto} id="getPhoto" />
                    </div>
                ...
                <div className="AboutButtons">
                    <input className="success" type="submit" value="Guardar" />
                </div>
            </form>
        </div>
    )
